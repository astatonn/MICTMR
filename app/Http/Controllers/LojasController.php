...
class LojasController extends Controller
{
    ...
    public function submit_nova_loja(Request $request)
    {
        ...
        $request->validate(
            //rules
            [
                'imagem_loja'           => ['required', 'image', 'mimes:jpg,bmp,png', 'max:512'],
                ...
                'endereco_loja'         => ['required', 'string', 'min:5', 'max:255'],
            ],
            //messages
            [
                // ERROS DA IMAGEM
                ...
                'imagem_loja.mimes'     => 'O formato deve ser jpg, bmp,png',
                ...
                // ERROS DO ENDEREÇO
                'endereco_loja.required'    => 'O endereço é obrigatório',
                'endereco_loja.min'         => 'O endereço deve ter no mínimo 5 caracteres',
                'endereco_loja.max'         => 'O endereço deve ter no máximo 255 caracteres',
            ]
        );
       ...
       lojas::firstOrCreate(
            ...
            [
                ...
                'imagem'    => $nome_imagem,
                'endereco'  => $request->endereco_loja,
            ]
        );
    ...
    // ==========================================================================
    public function editar_loja(Request $request, $id)
    {
        $session = $request->session()->get("permissions");
        if ($session == null || $session == 2) {
            return redirect()->route('home');
    ...
    // ==========================================================================
    public function submit_editar_loja(Request $request)
    {
        $session = $request->session()->get("permissions");
        if ($session == null || $session == 2) {
            return redirect()->route('home');
        ...
        $decrypted = Crypt::decrypt($request->id_loja);
        $loja = lojas::find($decrypted);

        $loja->nome = $request->nome_loja;
        $loja->oriente = $request->oriente_loja;
        $loja->numero = $request->numero_loja;
        $loja->titulo = $request->titulo_loja;
        $loja->endereco = $request->endereco_loja;
        $loja->imagem = $nome_imagem;
    ...
    {       
        ...
        $decrypted = Crypt::decrypt($id);
        $detalhesDaLoja = lojas::find($decrypted);
        $listaUsuarios = User::where ('loja_id',$decrypted)->orderBy('situacao','desc')->get();
